...
public class AccountBean implements AccountRemoteLocal {
	...
	public void inicializarVillages(Account ac) throws IOException {
		/*
		 * tc = CoreDeamon.getConnection(ac.getServer(), ac.getLogin(), ac.getPass()); params = new
		 * ArrayList<NameValuePair>(); Map<String, Village> lvi = new HashMap<String, Village>();
		 * 
		 * CloseableHttpResponse response = tc.sendGET(Tactions.getUrl("t_login"));
		 * 
		 * Document doc = Jsoup.parse(response.getEntity().getContent(), "UTF-8", tc.server);
		 * 
		 * params.add(new BasicNameValuePair("login", doc .select("input[name=login]").first().val()));
		 * params.add(new BasicNameValuePair("name", ac.getLogin())); params.add(new
		 * BasicNameValuePair("password", ac.getPass())); params.add(new BasicNameValuePair("w", ""));
		 * 
		 * response = tc.sendPOST(Tactions.getUrl("t_default1"), params); doc = Jsoup
		 * .parse(response.getEntity().getContent(), "UTF-8", tc.server);
		 * 
		 * java.util.Iterator<Element> ite = doc.select("#sidebarBoxVillagelist")
		 * .select(".innerBox.content").select("li").iterator(); while (ite.hasNext()) { Village nvi =
		 * new Village(); Map<Integer, Building> lbui = new HashMap<Integer, Building>();
		 * 
		 * Element village = ite.next(); nvi.setName(village.select(".name").first().text());
		 * nvi.setId(village.select("a").first().attr("href") .replaceAll("\\D+", ""));
		 * nvi.setX(village.select(".coordinateX").first().text() .replaceAll("\\D+", ""));
		 * nvi.setY(village.select(".coordinateY").first().text() .replaceAll("\\D+", ""));
		 * lvi.put(nvi.getId(), nvi);
		 * 
		 * ExecutorService es = Executors.newCachedThreadPool(); for (int i = 1; i <= 40; i++) {
		 * Building bb = new Building(); es.execute(new EventGetBuilding(ac.getLogin(), i, Integer
		 * .valueOf(nvi.getId()), bb)); lbui.put(i, bb); }
		 * 
		 * es.shutdown();
		 * 
		 * try { boolean finshed = es.awaitTermination(30, TimeUnit.SECONDS); } catch
	...
	public String initialize(String login, String pass, String Server) {
		...
		try {
			...
			if (doc.select(".ERROR.LTR") != null && !doc.select(".ERROR.LTR").text().trim().equals("")) {
			...
			for (String legion : doc.select(".nation").first().classNames()) {
				if (CoreActions.getUrl(legion) != null && !legion.equals("nation")) {
	...
	public Village[] obtenerVillas(String hashlogin) throws IOException {
		...
		java.util.Iterator<Element> ite = doc.select("#sidebarBoxVillagelist").select(".innerBox.content").select("li").iterator();
		while (ite.hasNext()) {
			...
			/*
			 * ExecutorService es = Executors.newCachedThreadPool(); for (int i = 1; i <= 40; i++) {
			 * Building bb = new Building(); es.execute(new EventGetBuilding(ac.getLogin(), i,
			 * Integer.valueOf(nvi.getId()), bb)); lbui.put(i, bb); }
			 * 
			 * es.shutdown();
			 * 
			 * try { boolean finshed = es.awaitTermination(30, TimeUnit.SECONDS); } catch
